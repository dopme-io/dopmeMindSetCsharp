# C# 14.0 — Roteiro MindSet

**Versão da linguagem:** 14.0  
**Contexto:** **novembro de 2025**; alinhado a **.NET 10** e à **cadência anual** de releases. *Extension members*, atribuição **null-condicional**, **`nameof`** com genéricos não ligados, mais conversões implícitas para **`Span<T>`** / **`ReadOnlySpan<T>`**, modificadores em parâmetros de **lambdas** simples, propriedades com **`field`**, **eventos** e **construtores** parciais, e operadores de **atribuição composta** definidos pelo utilizador.

Este roteiro usa o **SDK .NET atual** para exemplos. O `.csproj` fixa **`LangVersion` 14**. Inclui **cópia local** dos guias **13.0** e anteriores na mesma pasta. Não existe projeto MindSet **C# 6.0**; ver [csharp-7-0-overview.md](csharp-7-0-overview.md).
