# C# 9.0 — Roteiro MindSet

**Versão da linguagem:** 9.0  
**Contexto:** **novembro de 2020**; lançada com **.NET 5** — versão **predefinida** da linguagem para assemblies que visam **.NET 5**. Reforça **menos cerimónia**, **dados separados de algoritmos** (records, `init`) e **patterns** mais ricos (relacionais, `and`/`or`/`not`, `is not null`).

Este roteiro usa o **SDK .NET atual** para exemplos. Inclui **cópia local** dos guias **8.0** e anteriores na mesma pasta. Não existe projeto MindSet **C# 6.0**; ver [csharp-7-0-overview.md](csharp-7-0-overview.md).
