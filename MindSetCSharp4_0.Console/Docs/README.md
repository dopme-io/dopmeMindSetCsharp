# C# 4.0 — Roteiro MindSet

**Versão da linguagem:** 4.0  
**Contexto:** **abril de 2010**, com o **Visual Studio 2010** e o **.NET Framework 4**. A funcionalidade mais visível foi **`dynamic`** (ligação adiada via DLR). Para além disso: **argumentos nomeados** e **parâmetros opcionais**, **covariância/contravariância** (`out` / `in`) em interfaces genéricas, e **tipos de interop COM incorporados** (NoPIA), que simplificam implantação face a PIAs completos.

Este roteiro usa o **SDK .NET atual** para exemplos; os textos assinalam o que é próprio de **4.0** e o que são evoluções posteriores.

## Documentos C# 4.0 (ordem sugerida)

| Ordem | Tópico | Ficheiro |
|------:|--------|----------|
| 0 | Visão geral | [csharp-4-0-overview.md](csharp-4-0-overview.md) |
